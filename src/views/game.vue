<template>
  <div class="game">
      ...
      <div class="head">
        <button @click="back"> 返回 </button> <div>2048</div>
      </div>
      <div class="score">
        {{score}}
      </div>
      ...
      <div class="games" @touchstart.prevent="start" @touchend.stop.prevent="end" id="change">
          <div v-for="(e,i) in num" :key="i" :class="e>8?`text_white bg${e>0?Math.round(Math.log(e)/Math.log(2)):1}`:`text_dark  bg${e>0?Math.round(Math.log(e)/Math.log(2)):1}`">{{e}}</div>
      </div>
...
<style lang="scss" scoped>
    ...
    @for $i from 1 through 14 {
        .bg#{$i}{
            background-color: rgba(255, (275-20*$i), (275-20*$i),.8);
        }
    }
    .big{
    ...
    .game{
        ...
        background-image: linear-gradient(to top,rgb(226, 213, 98) 0%,rgb(214, 180, 30) 100%);
        div.bg{
            &:first-child{
                width: 100vw;height: 100vh;
                filter: blur(4px);
        ...
        div.end{
                ...
                font-size: 1.3rem;
                position: absolute;
                margin-left: -30vw;margin-top: -10vw;
                left:50%;top: 50%;
                color: #eee;
                display: flex;
...
export default {
    ...
    methods:{
        ...
        start(e){
            ...
            if(Math.abs(this.x1-x2)>Math.abs(this.y1-y2)){
                if(this.x1>x2){
                    for(var j=0;j<3;j++){
    ...
    watch:{
        num(){
            if(this.num.every((e,i)=>{return e>0})){
                this.num.forEach((e,i,arr)=>{
                    for(let m=0;m<4;m++){
                        if(i!=4*m){
                            if(arr[i]==arr[i-1]){
                                return
                            }else if(arr[i]==arr[i+4]&&i<12){
                                return
                            }else if(arr[0]!=arr[4]&&arr[4]!=arr[8]&&arr[8]!=arr[12]){
                                this.ended=true;
                                this.$store.commit('highScore',this.score)
                            }
                        }
